...
Для проєктів використовується рольова модель з двома ролями: `OWNER` та `MEMBER`.

У поточній реалізації будь-який учасник проєкту (незалежно від ролі) може створювати, редагувати, видаляти задачі та призначати виконавця. Обмеження на рівні ролей стосуються лише управління самим проєктом та його учасниками — наприклад, керувати складом учасників і змінювати ролі можуть тільки користувачі з роллю `OWNER`.

Таке рішення було прийнято в межах тестового завдання, щоб зосередитися на основній бізнес-логіці задач, а не на деталізованій матриці прав.

Модель permissions може бути розширена в майбутньому, наприклад, додаванням ролей:
